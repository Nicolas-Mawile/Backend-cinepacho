...
async def editar_multiplex(
    id: uuid.UUID,
    datos: MultiplexUpdate,
    repo: MultiplexRepository = Depends(get_repository),
    _: dict = Depends(get_current_admin_general),
):
    ...
    actualizado = repo.actualizar(id, datos.model_dump(exclude_none=True))
    return actualizado


@router.delete("/{id}", status_code=204)
async def eliminar_multiplex(
    id: uuid.UUID,
    repo: MultiplexRepository = Depends(get_repository),
    _: dict = Depends(get_current_admin_general),
):
    ...
    # En el sistema académico hacemos soft delete
    repo.desactivar(id)


@router.patch("/{id}/estado", response_model=MultiplexResponse)
async def cambiar_estado_multiplex(
    id: uuid.UUID,
    activo: bool,
    repo: MultiplexRepository = Depends(get_repository),
    _: dict = Depends(get_current_admin_general),
    ...
    if not existente:
        raise HTTPException(404, "Multiplex no encontrado")
    return repo.actualizar(id, {"activo": activo})